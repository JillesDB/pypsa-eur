...
if __name__ == "__main__":
    ...
    for dataset in ["corine", "luisa"]:
        ...
        # Pass path (not open handle) so atlite stores it as str; the crs=3035
        # kwarg lets open_files() fix malformed LOCAL_CS metadata in workers.
        dataset_path = snakemake.input[dataset]

        if isinstance(settings, dict) and "grid_codes" in settings:
            codes = settings["grid_codes"]
            excluder.add_raster(
                dataset_path, codes=codes, invert=True, crs=3035, **kwargs
            )
        if settings.get("distance", 0.0) > 0.0:
            codes = settings["distance_grid_codes"]
            buffer = settings["distance"]
            excluder.add_raster(
                dataset_path, codes=codes, buffer=buffer, crs=3035, **kwargs
            )
